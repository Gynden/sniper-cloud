...
#   POST /api/config       -> ajusta config (stake, gales, thresholds)
#   POST /api/reset        -> zera histórico
#
# Deploy: Render usa a variável de ambiente PORT; bind em 0.0.0.0

import os, math, time, json, statistics, random
...
# Serve arquivos na própria raiz do repo (index.html)
...
def strat_cluster_bias():
    ...
    r = last5.count("red"); b = last5.count("black")
    out = {}
    if r >= 3: out["red"] = 1.0
...
def decide_signal():
    ...
    if active_signal and active_signal.get("status") == "running":
        return None  # bloqueado enquanto houver gales

    votes = compute_votes()
    scores = {
        "red":   votes.get("red", 0.0),
        "black": votes.get("black", 0.0),
        "white": votes.get("white", 0.0) * 1.15,
    }
...
def evaluate_active_signal(new_result: str):
    ...
    if hit:
        ...
        if s["gale_step"] < s["max_gales"]:
            s["gale_step"] += 1  # tenta novo gale, mantém running
        else:
...
# (Opcional) simulação de rodada no Render para testes
@app.post("/api/demo_tick")
def demo_tick():
    u = random.random()
    result = "red" if u < 0.45 else ("black" if u < 0.90 else "white")
    return push_round.__wrapped__()  # reutiliza lógica do push; precisa do body
    # Obs.: para chamar: POST /api/demo_tick com {"result":"red|black|white"}
