...
DATA_SOURCE = "tipminer"           # só informativo

# ========================= Estado básico =========================
history = deque(maxlen=HISTORY_MAX)   # roleta (0..14)
signals = deque(maxlen=2000)          # log de sinais (para estatística/relatório)
last_snapshot = []
bot_on = False
...
class SpectraLearner:
    """
    IA simplificada (contagens tipo 'beta'):
      - p_red: razão de RED vs BLACK (ignorando brancos)
      - p_white: frequência adaptativa do branco (com suavização)
    """
    def __init__(self):
        self.red_s = 1.0; self.red_f = 1.0         # sucessos/falhas p/ RED
        self.white_s = 1.0; self.white_f = 13.0    # branco é raro
        self.last_preds = deque(maxlen=10)         # p_major para entropia/mercado
        self.recent_outcomes = deque(maxlen=60)    # wins de entradas
    ...
    def update_from_spin(self, outcome_color):
        """atualiza a distribuição ao final de um giro"""
        if outcome_color == "W":
            self.white_s += 1.0
        else:
            self.white_f += 1.0
...
# ========================= Probabilidades (para UI) ======================
def estimate_probs_ai(seq):
    # usa learner (p_red, p_white), renormaliza R/B para (1-pW)
...
def append_signal_entry(mode, target, step, status="open", came=None,
    ...
    mismatch = status in ("WIN","LOSS") and came_color and (came_color==target and status=="LOSS")
    signals.appendleft({
        "ts": now_local_hms(),
        "iso_ts": now_iso_utc(),
...
def hourly_assertivity(last_hours=24, kind_filter=None):
    """
    Retorna pontos por hora (últimas 'last_hours'):
      [{"t":"2025-10-21T10:00Z","win":x,"loss":y,"acc":0.54}, ...]
    kind_filter: "CORES" | "BRANCO" | None
    """
    ...
    for s in signals:
        ...
        try:
            ts = datetime.fromisoformat(s["iso_ts"])
        except Exception:
            continue
...
def try_open_trade_if_needed():
    ...
    # Push do nível de certeza p/ "mercado ruim"
    p_red, p_white = learner.predict()
...
def process_new_number(n):
    ...
    tick_cooldowns()
    learner.update_from_spin(color_code(n))  # IA aprende de TODO giro

    if not open_trade:
        try_open_trade_if_needed()
        return

    # cancelamento se mudou modo
...
def state():
    ...
    market_bad = (wr60 < 0.48) or learner.entropy_high()
    ...
    out = {
        ...
        "entropy_high": learner.entropy_high()
    }
    return jsonify(out)
...
def stats():
    return jsonify({
        ...
        "mercado_ruim": (learner.winrate60() < 0.48) or learner.entropy_high(),
...
@app.route("/history")
def history_api():
    # filtros: ?hours=24&kind=CORES/BRANCO
    try:
...
@app.route("/report")
def report():
    """Página simples para 'Imprimir em PDF' no navegador."""
    rows = []
    ...
    <p>Winrate(60): {round(learner.winrate60()*100,1)}% — Mercado: {"Ruim" if (learner.winrate60()<0.48 or learner.entropy_high()) else "OK"}</p>
