...
class HistoryPayload(BaseModel):
    history: List[SpinColor]  # oldest -> newest
...
class Config(BaseModel):
    stop_win_whites: int = 5
    stop_loss_tentativas: int = 10
    max_tentativas_por_ciclo: int = 2  # 5º e 8º
    hora_inicio: int = 0
    hora_fim: int = 23
    min_score_para_operar: int = 40
    cooldown_ciclos_ruins: int = 2
    cooldown_giros: int = 40
    janela_mercado: int = 30  # giros para analisar score
...
# Estado simples em memória (um único "perfil" global)
CURRENT_STATS = Stats()
CONFIG = Config()
...
def calcular_score(history: List[SpinColor], stats: Stats, cfg: Config) -> int:
    ...
    if dist is None:
        score -= 10  # começo de dia sem referência
    else:
        if dist < 3:
            score -= 15
        elif 3 <= dist <= 8:
            ...
            score -= 5  # muito seco pode indicar dia estranho

    # 2) sequências malucas de cores (só vermelho/preto)
    same_streak = 1
    max_streak = 1
    last = janela[0]
    for c in janela[1:]:
    ...
    if max_streak >= 10:
        ...
        score += 5  # mercado alternando mais

    # 3) desempenho recente (tentativas vs whites)
    ...
    # 4) penalizar se já tomou muitas perdas
...
def analisar_regra_5e8(history: List[SpinColor], stats: Stats, cfg: Config):
    ...
    # Próximo giro será (giros_desde_white + 1) após o white
    proximo_numero = giros_desde_white + 1
...
def decidir(payload: HistoryPayload):
    ...
    if acao_stop is not None:
        # score ainda calculado pra painel
...
def atualizar_resultado(result: ResultPayload):
    ...
    # Se teve muitos ciclos ruins, ativa cooldown
...
@app.post("/tick")
def tick():
    """Endpoint opcional para o SubBot chamar a cada giro, só para reduzir cooldown_giros_restantes."""
