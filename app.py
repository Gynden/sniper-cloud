...
# Modo de operação: "colors" (padrão) | "white"
...
config = dict(
    ...
    entry_mode="colors",          # "colors" ou "white"
    autoclear_on_finish=True,     # limpa o painel ao terminar a rodada/gales
)
...
_last_finished: Optional[Dict] = None   # enviado 1x no /api/state para preencher a tabela
...
class StrategyGA:
    ...
    def evaluate_and_update(self, seq_colors: List[str]):
        if len(seq_colors) < 60: return
        while len(self.pool) < 30: self.pool.append(_random_strategy(self.generation))
        scored = []
...
def _finish_signal(result: str):
    """Finaliza e (se configurado) limpa painel, preservando uma cópia para o histórico do front."""
    ...
    # guarda 1x para o front adicionar na tabela
    _last_finished = dict(active_signal)
    # limpa painel imediatamente se configurado
    if config.get("autoclear_on_finish", True):
        active_signal = None

def apply_result(new_color: str):
    """Conta resultado automaticamente (sem confirmar)."""
    ...
    # Errou: tenta gale ou finaliza LOSS
...
def maybe_decide_signal():
    """Emite novo sinal se não houver um rodando."""
    global active_signal
...
def state():
    ...
    payload = {
        ...
        "active_signal": active_signal,             # None se terminou e autoclear ativo
    ...
    # limpa last_finished após entregar
    _last_finished = None
    return jsonify(payload)

@app.post("/api/reset")
def reset_all():
    global history, metrics, active_signal, _ai, _ga, _last_finished
    history.clear()
...
def update_config():
    ...
    allowed = {"stake", "max_gales", "confidence_min", "invert_mapping", "entry_mode", "autoclear_on_finish"}
