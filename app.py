...
def maybe_decide_signal():
    ...
    # gates por confiança (aplicável em hybrid/manual)
    if conf < config["confidence_min"]:
        return
    # white pode opcionalmente ter threshold diferente (use se quiser)
    active_signal = {
        "created_at": now_iso(),
        "color": color,
...
def apply_result(new_color: str):
    ...
    if not active_signal or active_signal.get("status")!="running":
        return
    hit = (new_color == active_signal["color"])
    if hit or active_signal["color"]=="white" and new_color=="white":
...
def push_round():
    ...
    # atualiza histórico, aplica feedback e decide próximo
